...
def p2e_and_blend_torch(
    ...
    if cache_key not in p2e_and_blend_torch._grid_cache:
        # FIX B: Compute grid on CPU for deterministic results.
        # CUDA matmul (xyz @ R) is non-deterministic; near the nadir where
        # z_cam ≈ 0, the division x_cam/z_cam amplifies tiny FP variations
        # causing random blurriness. Grid is moved to GPU only at the end.
        grid_device = torch.device("cpu")

        # Create equirectangular coordinate grid
        eq_y, eq_x = torch.meshgrid(
            torch.arange(h_eq, device=grid_device, dtype=torch.float32),
            torch.arange(w_eq, device=grid_device, dtype=torch.float32),
            indexing="ij",
        )
        ...
        # Rotation angles
        u = torch.deg2rad(
            torch.tensor(float(u_deg), device=grid_device, dtype=torch.float32)
        )
        v = torch.deg2rad(
            torch.tensor(float(-v_deg), device=grid_device, dtype=torch.float32)
        )

        cu, su = torch.cos(u), torch.sin(u)
        cv, sv = torch.cos(v), torch.sin(v)

        z0 = torch.tensor(0.0, device=grid_device, dtype=torch.float32)
        o1 = torch.tensor(1.0, device=grid_device, dtype=torch.float32)
        ...
        # Apply rotation (deterministic on CPU)
        R = Ry @ Rx
        xyz_cam = xyz @ R
        ...
        # FOV calculations
        tan_h = torch.tan(
            torch.deg2rad(torch.tensor(fov_w, device=grid_device, dtype=torch.float32)) / 2.0
        )
        tan_v = torch.tan(
            torch.deg2rad(torch.tensor(fov_h, device=grid_device, dtype=torch.float32)) / 2.0
        )

        # Check if within FOV
        in_fov = valid & (x_p.abs() <= tan_h) & (y_p.abs() <= tan_v)

        # Convert to pixel coordinates (original formula)
        ...
        # Create grid [1, H, W, 2] and move to GPU
        grid = torch.stack([grid_x, grid_y], dim=-1).unsqueeze(0).to(device)

        # Create mask (HW, uint8 with values 0 or 255) and move to GPU
        mask_t = (in_fov.to(torch.uint8) * 255).to(device)
