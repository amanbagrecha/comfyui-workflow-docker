...
def _pad_to_tile_grid(rgb_u8, rows, cols):
    ...
    if pad_right == 0 and pad_bottom == 0:
        return rgb_u8, 0, 0
    return np.pad(rgb_u8, ((0, pad_bottom), (0, pad_right), (0, 0)), mode="edge"), pad_right, pad_bottom
...
def _infer_mask(tile_pil, prompt, threshold):
    ...
    inputs = _PROCESSOR(images=tile_pil, text=prompt.strip(), return_tensors="pt").to(_CFG["device"])
    with torch.no_grad():
...
def _process_one(task):
    ...
    if cfg["resize_width"] > 0 and cfg["resize_height"] > 0:
        rgb_u8 = np.asarray(
            Image.fromarray(rgb_u8).resize((cfg["resize_width"], cfg["resize_height"]), LANCZOS),
            dtype=np.uint8,
        )

    rgb_u8, pad_right, pad_bottom = _pad_to_tile_grid(rgb_u8, cfg["tile_rows"], cfg["tile_cols"])
    h, w = rgb_u8.shape[:2]
    windows = _tile_windows(
        w, h, cfg["tile_rows"], cfg["tile_cols"],
        cfg["overlap_ratio"], cfg["overlap_x"], cfg["overlap_y"],
    )

    stitched = np.zeros((h, w), dtype=np.float32)
    for x1, y1, x2, y2 in windows:
        tile_pil = Image.fromarray(rgb_u8[y1:y2, x1:x2], mode="RGB")
        ...
        stitched[y1:y2, x1:x2] = np.maximum(stitched[y1:y2, x1:x2], tile_mask[: y2 - y1, : x2 - x1])
    ...
    return {"input": str(in_path), "output": str(out_path), "elapsed_sec": time.perf_counter() - t0}


@click.command(context_settings=dict(help_option_names=["-h", "--help"]))
@click.option("-i", "--input-dir", "input_path", required=True, type=click.Path(path_type=Path, exists=True))
@click.option("-o", "--output-dir", "output_dir", required=True, type=click.Path(path_type=Path))
...
@click.option("--model-path", default=DEFAULT_MODEL_PATH, show_default=True, type=str,
              help="HuggingFace hub ID (e.g. 'facebook/sam3') or local model directory.")
@click.option("--device", default="auto", show_default=True, type=click.Choice(["auto", "cuda", "cpu"]))
...
@click.option("--overlap-x", default=0, show_default=True, type=int)
...
def main(
    input_path, output_dir, pattern, recursive, skip_existing, workers,
    model_path, device, resize_width, resize_height,
    tile_rows, tile_cols, overlap_ratio, overlap_x, overlap_y,
    sky_prompt, sky_threshold, glare_prompt, glare_threshold,
    mask_threshold, square_output,
):
    t0 = time.perf_counter()
    ...
    tasks = [
        ...
        for f in in_files
        if not (skip_existing and (output_dir / f.relative_to(in_root).with_suffix(".png")).exists())
    ]
    ...
    cfg = dict(
        ...
        overlap_ratio=overlap_ratio, overlap_x=overlap_x, overlap_y=overlap_y,
        sky_prompt=sky_prompt, sky_threshold=sky_threshold,
        glare_prompt=glare_prompt, glare_threshold=glare_threshold,
        mask_threshold=mask_threshold, square_output=square_output,
    )

    click.echo(f"SAM3 tiled mask: input={len(in_files)} pending={len(tasks)} workers={workers} device={device}")
    click.echo(f"resize={resize_width}x{resize_height} tiles={tile_rows}x{tile_cols} sky='{sky_prompt}'({sky_threshold}) glare='{glare_prompt}'({glare_threshold})")
    ...
    if workers == 1:
        ...
        for task in tasks:
            ...
            click.echo(f"OK   {Path(result['input']).name} ({result['elapsed_sec']:.2f}s)")
    else:
        ctx = mp.get_context("spawn")
        with ProcessPoolExecutor(max_workers=workers, initializer=_worker_init, initargs=(cfg,), mp_context=ctx) as ex:
    ...
    click.echo(f"done={len(results)} fail={len(failures)} total={time.perf_counter() - t0:.2f}s avg={avg:.2f}s")
